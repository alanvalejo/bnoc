...
def plot_network(graph, options):
	...
	while len(types) < 10:
		types += types
	if options.layout_name in ['kk', 'fr']:
		for layer in range(graph['layers']):
			vertices = graph.vs.select(type=layer)['index']
			for vertex in vertices:
				graph.vs[vertex]['vertex_shape'] = types[layer]
	if options.overlapping is not None and options.overlappin_paint:
		print options.overlapping_shape
...
if __name__ == '__main__':
	...
	parser = args.setup_parser(current_path + '/args/plot-network.json')
	...
	if options.membership:
		...
		options.membership = graph.vs['membership']

		for layer in range(graph['layers']):
			vertices = graph.vs.select(type=layer)['index']
			options.membership = numpy.array(options.membership)
